...
export function MitraAddDialog() {
  ...
  const { mutateAsync: createMitra, isPending } = useCreateMitra();
  ...
  const handleCreate = async (values: CreateMitraInput) => {
    ...
    toast.promise(createMitra(formData), {
      loading: "Membuat mitra...",
      success: () => {
        setOpen(false);
        form.reset();
        setPreviewLogo(null);
        return "Mitra berhasil dibuat";
      },
  ...
  return (
    <EntityDialog
      ...
      title="Tambah Mitra"
      description="Isi detail mitra di bawah ini"
      isPending={isPending}
